...
def add_auth():
  ...
  if "token" not in st.session_state:
      ...
      st.markdown(f"Please [login]({authorization_url}) and authorize the app.")
      token = st.text_input("Enter the token from the redirect URL:")
      if token:
          try:
              flow.fetch_token(code=token)
              st.session_state.token = flow.credentials.to_json()
              st.session_state.email = build("oauth2", "v2", credentials=flow.credentials).userinfo().get().execute()['email']
              st.experimental_rerun()
          except Exception as e:
              st.error(f"Authentication failed: {str(e)}")
  else:
      credentials = Credentials.from_authorized_user_info(eval(st.session_state.token))
      if credentials.expired:
          flow.credentials = credentials
          flow.refresh_token(google.auth.transport.requests.Request())
          st.session_state.token = flow.credentials.to_json()
      st.session_state.email = build("oauth2", "v2", credentials=credentials).userinfo().get().execute()['email']
