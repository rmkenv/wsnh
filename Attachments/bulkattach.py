...
def add_auth():
    ...
    if "credentials" not in st.session_state:
        # If we don't have credentials, start the OAuth flow
        authorization_url, _ = flow.authorization_url(prompt="consent")
        st.markdown(f"Please [login]({authorization_url}) and authorize the app.")
    else:
        # If we have credentials, use them to authenticate
        credentials = Credentials.from_authorized_user_info(st.session_state["credentials"])
        drive_service = build("drive", "v3", credentials=credentials)
        # You can now use drive_service to interact with Google Drive
        st.session_state.email = credentials.id_token['email']
